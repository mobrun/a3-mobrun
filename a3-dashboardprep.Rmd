...
You should have already received an API key. Copy and paste it into the code below. This chunk sets the key as a back-end environment variable that the `censusapi` package then knows how to use.
...
Below, the first `getCensus()` call is populating a new dataframe called `incometract16` with the contents of the ACS 2016 5-yr estimate for median household income (the only thing you have to add to this chunk for it to run is the correct variable ID for median household income. You should be able to find it in https://api.census.gov/data/2016/acs/acs5/variables.html), for tracts within 9 counties in California (FIPS ID 06). The 9 counties have unique IDs, 001,013,041,055,075,081,085,095,097, which you would find yourself on a Census website. The specific structure of the function call can be learned from the package creator's documentation.

The second `getCensus()` call is grabbing just county results for the 9 counties.

```{r load census data}
incometract16 <- getCensus(name = "acs/acs5", vintage = "2016", vars = "INSERTVARIABLEHERE",
region = "tract:*", regionin = "state:06+county:001,013,041,055,075,081,085,095,097")

colnames(incometract16)[4] <- "Tract"

incomecounty16 <- getCensus(name = "acs/acs5", vintage = "2016", vars = "INSERTVARIABLEHERE",
